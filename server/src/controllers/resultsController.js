...
// routes/results.js
export const getUpdatedResultsById =  asyncHandler(async (req, res) => {
  const { results, totalVotes, correctionNote, status } = req.body;

  const update = {
    ...(results    !== undefined && { results }),
    ...(totalVotes !== undefined && { totalVotes }),
    ...(status     !== undefined && { status }),
    // Append to correction history instead of overwriting
    ...(correctionNote && {
      $push: {
        correctionHistory: {
          note:        correctionNote,
  ...
  const updated = await ElectionResult.findByIdAndUpdate(
    req.params.id,
    { $set: update, ...(correctionNote && { $push: update.$push }) },
    { new: true }
  );
