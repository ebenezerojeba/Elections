...
export const registerRules = [
  ...
  body('password')
    ...
    .matches(/\d/)
    .withMessage('Password must contain a number'),
  // REMOVED: body('pollingUnit')
  // ADDED: lcda + ward ObjectId refs sent from the register form cascading selects
  body('lcda')
...
export const resultRules = [
  // REMOVED: body('pollingUnit') — comes from req.user.ward via JWT, not the body
  body('results')
    .isArray({ min: 1 })
    .withMessage('results must be a non-empty array'),
  body('results.*.party')
