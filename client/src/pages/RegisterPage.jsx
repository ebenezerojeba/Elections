...
//   fetchLcdas()
//     .then(data => {
//       console.log('Total LCDAs loaded:', data.length); // Should be 57
//       setLcdas(data);
//     })
//     .catch(() => toast.error('Failed to load LCDAs'))
//     .finally(() => setLcdaLoading(false));
  ...
  const onSubmit = async (values) => {
    ...
    try {
      await registerAgent({
        ...
        password: values.password,
        lcda:     values.lcda,
        // No ward — agent picks ward per submission
      });
      toast.success('Account created! Welcome, Agent.');
      navigate('/agent');
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="w-full max-w-lg animate-fade-up">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 text-sm">
            Register to start submitting election results for your LCDA
          </p>
        </div>
        ...
        <div className="card">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
            ...
            <div>
              ...
              <select
                className={clsx('input bg-white', errors.lcda && 'input-error', !selectedLcda && 'text-slate-400')}
                disabled={lcdaLoading}
                {...register('lcda', { required: 'Please select your LCDA' })}
              >
                <option value="">
            ...
            {lcdaName && (
              <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs">
                ...
                <span className="text-slate-500">Assigned to LCDA</span>
                <span className="font-mono font-medium text-ink-900">{lcdaName}</span>
                <span className="text-slate-400 ml-auto">Ward selected per submission</span>
              </div>
            )}
