...
function FeedItem({ result, isNew }) {
  const topParty = [...(result.results || [])].sort((a, b) => b.votes - a.votes)[0];

  return (
    <div
      className={clsx(
        'flex items-start gap-2.5 p-2.5 sm:p-3 rounded-xl border transition-all duration-500',
        isNew
          ? 'bg-vote-50 border-vote-200 animate-slide-in'
          : 'bg-white border-slate-100'
      )}
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-start justify-between gap-1.5">
          <p className="font-mono font-bold text-ink-900 text-xs truncate leading-snug">
            {result.pollingUnit}
          </p>
          <span className="text-xs text-slate-400 shrink-0 tabular-nums leading-snug">
            {formatDistanceToNow(new Date(result.submittedAt), { addSuffix: true })}
          </span>
        </div>

        {/* Party pills — max 3 on mobile, 4 on sm */}
        <div className="flex flex-wrap gap-1 mt-1.5">
          ...
          {result.results?.[3] && (
            <span
              ...
              style={{
                background: partyColor(result.results[3].party).light,
                color:      partyColor(result.results[3].party).bg,
              }}
            >
              {result.results[3].party} {result.results[3].votes.toLocaleString()}
            </span>
          )}
          {result.results?.length > 4 && (
            <span className="text-xs text-slate-400">+{result.results.length - 4}</span>
          )}
        </div>

        {/* Agent */}
        <p className="text-xs text-slate-400 mt-1 truncate">by {result.agentName}</p>
      </div>
    </div>
  );
}
...
export default function LiveFeed({ results = [], newestId = null }) {
  ...
  return (
    <div className="card h-full flex flex-col">
      ...
      {/* Feed list */}
      <div
        className="flex-1 overflow-y-auto space-y-1.5 sm:space-y-2 pr-0.5
                   scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent"
      ...
      {/* Footer */}
      <p className="text-xs text-slate-300 text-center mt-3 pt-3 border-t border-slate-50 shrink-0">
        {sorted.length} unit{sorted.length !== 1 ? 's' : ''} reported
      </p>
    </div>
  );
