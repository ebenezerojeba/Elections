...
function PartyRow({ party, votes, totalVotes, rank, isLeader }) {
  ...
  return (
    <div className={clsx('group py-1', isLeader && 'animate-fade-up')}>
      {/* Top line: rank + name + votes */}
      <div className="flex items-center justify-between mb-1.5 gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {/* Party pill */}
          <span
            className="font-mono font-bold text-sm px-2 py-0.5 rounded-lg truncate"
            style={{ background: color.light, color: color.bg }}
          >
            {party}
          </span>

          {/* Leading badge — hidden on xs to prevent overflow */}
          {isLeader && (
            <span className="hidden sm:inline text-xs bg-gold-50 text-gold-700 px-2 py-0.5 rounded-full font-medium shrink-0">
              Leading
            </span>
          )}
        </div>
        ...
        <div className="text-right shrink-0">
          ...
          <span className="text-slate-400 text-xs ml-1.5 tabular-nums">
            {pct.toFixed(1)}%
          </span>
        </div>
      </div>
...
export default function PartyLeaderboard({ parties = [], grandTotal = 0 }) {
  if (!parties.length) {
    return (
      <div className="card">
        <h2 className="font-display font-bold text-ink-900 mb-6">Party standings</h2>
        <div className="py-12 text-center text-slate-400 text-sm">No results submitted yet</div>
      </div>
    );
  }
    ...
    <div className="card">
      <div className="flex items-center justify-between mb-5">
        ...
        <span className="text-xs text-slate-400 font-mono tabular-nums">
          {grandTotal.toLocaleString()} votes
        </span>
      </div>

      <div className="divide-y divide-slate-50">
        {sorted.map((p, i) => (
          <div key={p.party} className="py-2.5 first:pt-0 last:pb-0">
            <PartyRow
              party={p.party}
              votes={p.totalVotes}
              totalVotes={grandTotal}
