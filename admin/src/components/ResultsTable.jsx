...
const TH_BASE = {
  padding: '10px 12px', fontFamily: 'var(--mono)', fontSize: 9,
  letterSpacing: '0.16em', textTransform: 'uppercase', fontWeight: 500,
  color: 'var(--muted)', borderBottom: '1px solid var(--border-2)',
  textAlign: 'left', whiteSpace: 'nowrap', userSelect: 'none',
};

const TD_BASE = {
  padding: '11px 12px', borderBottom: '1px solid var(--border)', verticalAlign: 'middle',
};
...
function PartyPill({ party, votes, compact = false }) {
  ...
  const displayVotes   = compact && votes >= 1000
    ? `${(votes / 1000).toFixed(1)}k` : votes.toLocaleString();
  return (
    <span style={{
      display: 'inline-flex', alignItems: 'center', gap: compact ? 3 : 5,
      padding: compact ? '2px 6px' : '2px 8px', borderRadius: 4,
      background: `${bar}22`, border: `1px solid ${bar}55`,
      fontFamily: 'var(--mono)', fontSize: compact ? 9 : 10,
      whiteSpace: 'nowrap', color: label,
    }}>
        ...
        return (
          <div key={p.party}>
            ...
            <div style={{ height: 4, background: 'rgba(255,255,255,0.06)', borderRadius: 2, overflow: 'hidden' }}>
              <div style={{
                width: `${pct}%`, height: '100%', background: bar, borderRadius: 2,
  ...
  return (
    <div style={{
      ...
      {result.correctionNote && (
        <span style={{ color: '#C9A84C', width: '100%', marginTop: 4 }}>
          ✎ {result.correctionNote}
        </span>
      )}
    </div>
...
function MobileCard({ r, isNewest, isExpanded, onToggle, onEditRequest }) {
  ...
  return (
    <div onClick={onToggle} style={{
      ...
      border: `1px solid ${isNewest ? 'rgba(201,168,76,0.45)' : isExpanded ? 'var(--border-2)' : 'var(--border)'}`,
      background: isNewest ? 'rgba(201,168,76,0.06)' : isExpanded ? 'var(--surface-2)' : 'var(--surface-3)',
      overflow: 'hidden', cursor: 'pointer', transition: 'border-color 0.2s, background 0.2s',
    }}>
      <div style={{
        display: 'flex', alignItems: 'flex-start',
        justifyContent: 'space-between', gap: 8, padding: '11px 14px 8px',
      }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          <div style={{ textAlign: 'right' }}>
            ...
            <StatusControl
              resultId={r._id}
              status={r.status}
              onUpdated={(newStatus) => {/* handled by localResults in parent */}}
      ...
      {isExpanded && (
        <div style={{
          padding: '12px 14px 14px', borderTop: '1px solid var(--border)',
          background: 'rgba(201,168,76,0.03)', animation: 'slide-up 0.2s ease both',
        }}>
...
function PageBtn({ label, onClick, disabled = false, active = false }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      fontFamily: 'var(--mono)', fontSize: 10, fontWeight: active ? 600 : 400,
...
export default function ResultsTable({ results = [], newestId = null }) {
  ...
  // Shadow the prop so status changes + corrections update in place
  ...
  const [editTarget,  setEditTarget]  = useState(null); // result being edited

  // Patch a single result in local state by id
  ...
  const statusCounts = useMemo(() => {
    const c = { all: localResults.length, pending: 0, verified: 0, rejected: 0 };
  ...
  return (
    <>
      <div ref={containerRef} style={{ display: 'flex', flexDirection: 'column', height: '100%', minWidth: 0 }}>
          ...
          {STATUS_TABS.map(tab => {
            ...
            return (
              <button key={tab} onClick={() => handleTabChange(tab)} style={{
                ...
                gap: 5, padding: bp === 'xs' ? '4px 9px' : '5px 12px', borderRadius: 6,
                ...
                <span style={{
                  ...
                  background: active ? (meta ? `${meta.color}22` : 'rgba(201,168,76,0.15)') : 'rgba(255,255,255,0.05)',
                  color: active ? (meta ? meta.color : 'var(--apc-gold)') : 'var(--muted)',
        ...
        {bp === 'xs' && (
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {paged.length === 0 ? (
              <div style={{ padding: '40px 0', textAlign: 'center', fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--muted)' }}>
              ...
              paged.map(r => (
                <MobileCard
                  ...
                  onEditRequest={(e) => { e?.stopPropagation(); setEditTarget(r); }}
                />
              ))
            )}
          </div>
        ...
        {bp !== 'xs' && (
          <div style={{ overflowX: 'auto', flex: 1 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 0 }}>
              <thead>
                <tr>
                  <th style={{ ...TH_BASE, cursor: 'pointer' }} onClick={() => toggleSort('pollingUnit')}>
                  ...
                  <th style={{ ...TH_BASE, cursor: 'pointer' }} onClick={() => toggleSort('totalVotes')}>
                  ...
                  {showSubmitted && (
                    <th style={{ ...TH_BASE, cursor: 'pointer' }} onClick={() => toggleSort('submittedAt')}>
                ...
                {paged.length === 0 ? (
                  <tr>
                    <td colSpan={colCount} style={{
                      ...TD_BASE, textAlign: 'center', padding: '48px 24px',
                      fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--muted)', border: 'none',
                    }}>
                      {search || statusTab !== 'all' ? 'No results match your filter' : 'No results yet'}
                    </td>
                  </tr>
                  ...
                  paged.map(r => {
                    ...
                    const rowBg      = isNewest ? 'rgba(201,168,76,0.06)' : isExpanded ? 'var(--surface-2)' : 'transparent';
                    const tdX        = { ...TD_BASE, borderBottom: isExpanded ? '1px solid transparent' : TD_BASE.borderBottom };

                    return (
                      <React.Fragment key={r._id}>
                        <tr onClick={() => toggleExpand(r._id)}
                          style={{ cursor: 'pointer', background: rowBg, transition: 'background 0.18s' }}
                          onMouseEnter={e => { if (!isNewest && !isExpanded) e.currentTarget.style.background = 'var(--surface-3)'; }}
                        ...
                        >
                          <td style={{ ...tdX, maxWidth: bp === 'sm' ? 140 : 220 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                              ...
                              <span style={{
                                fontFamily: 'var(--display)', fontSize: bp === 'sm' ? 11 : 12, fontWeight: 700,
                                color: isNewest ? 'var(--apc-gold2)' : 'var(--text)',
                                overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                              }}>
                                {r.pollingUnit}
                          ...
                          <td style={tdX}>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                              {topParties.map(p => (
                                <PartyPill key={p.party} party={p.party} votes={p.votes} compact={bp === 'sm'} />
                              ))}
                              {overflow > 0 && (
                                <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--muted)', alignSelf: 'center' }}>
                                  +{overflow}
                          ...
                          <td style={tdX}>
                            <span style={{
                              fontFamily: 'var(--mono)', fontSize: bp === 'sm' ? 12 : 13,
                          ...
                          {showStatus && (
                            <td style={tdX}>
                              <StatusControl
                                resultId={r._id}
                                status={r.status}
                                onUpdated={(newStatus) => patchLocal(r._id, { status: newStatus })}
                                onEditRequest={(e) => { e?.stopPropagation(); setEditTarget(r); }}
                              />
                            </td>
                          )}
                          ...
                          {showSubmitted && (
                            <td style={{ ...tdX, fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--muted)', whiteSpace: 'nowrap' }}>
                              {r.submittedAt
                        ...
                        {isExpanded && (
                          <ExpandedRow  result={r} colSpan={colCount} />
                        )}
                      </React.Fragment>
                    );
                  })
        ...
        {pageCount > 1 && (
          <div style={{
            ...
            paddingTop: 14, marginTop: 10, borderTop: '1px solid var(--border)',
            gap: 10, flexWrap: 'wrap',
          }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--muted)', whiteSpace: 'nowrap' }}>
              {bp === 'xs' ? `${safePage} / ${pageCount}` : `Page ${safePage} of ${pageCount} · ${filtered.length} results`}
            </span>
            <div style={{ display: 'flex', gap: 4, alignItems: 'center', flexWrap: 'wrap' }}>
              <PageBtn label="← Prev" disabled={safePage === 1} onClick={() => setPage(p => Math.max(1, p - 1))} />
              {bp !== 'xs' && buildPageWindow(safePage, pageCount).map((n, i) =>
                n === '…' ? (
                  <span key={`e${i}`} style={{ padding: '0 3px', fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--muted)' }}>…</span>
                ) : (
                  <PageBtn key={n} label={String(n)} active={n === safePage} onClick={() => setPage(n)} />
                )
              )}
              <PageBtn label="Next →" disabled={safePage === pageCount} onClick={() => setPage(p => Math.min(pageCount, p + 1))} />
            </div>
          </div>
        )}
      </div>

      {/* Edit modal — rendered outside the table div to avoid stacking context issues */}
      <EditResultModal
        result={editTarget}
        onClose={() => setEditTarget(null)}
        onSaved={(updated) => {
          patchLocal(updated._id, {
            results:        updated.results,
            totalVotes:     updated.totalVotes,
            status:         updated.status,         // will be 'pending'
